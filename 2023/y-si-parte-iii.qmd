...
estim_publico_con_mod_privado = predict(mod_sector_privado, ess[ess$treatment==1, ])

(media_estimada_sector_publico <- weighted.mean(estim_publico_con_mod_privado, ess$factotal[ess$treatment == 1]))


(media_observada_sector_publico <- weighted.mean(ess$outcome[ess$treatment == 1], ess$factotal[ess$treatment == 1]) )

(diferencia <- media_observada_sector_publico - media_estimada_sector_publico)



```

Y haciéndolo así se tendría que si los trabajadores  del sector público se cambiaran al privado, manteniendo el resto igual ganarían unos 5 euros menos al mes de media. 
