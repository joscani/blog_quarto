...
Este post es prácticamente idéntico al del año pasado. El script ha funcionado igual y lo único que he tardado un pelín más en hacerlo porque he cambiado de sistema operativo. He pasado hace poco a una distribución rolling release basada en `arch` y aún estoy dejando el sistema fino.  
...
* Usar un sistema operativo GNU/Linux, en mi caso Omarchy 3.2.3. Una distro basada en Arch linux
* `ffmpeg`    ffmpeg nos va a servir para poder bajar el video 
* `mplayer`   vamos a extraer 1 de cada 200 fotogramas del video
*  `imagemagick`  Con `imagemagick` vamos a cambiar el tamaño de los fotogramas, recortar el área dónde están los subtítulos, convertir a formato `tif` e invertir los colores para que el texto se vea mejor. 
* `parallel` Con `parallel` vamos a poder utilizar varios núcleos e hilos del ordenador en paralelo y poder hacer la modificación de los fotogramas y el reconocimiento óptico de caracteres más rápido. 
* `tesseract` Con `tesseract` se va a hacer el reconocimiento óptico del texto y guardar en ficheros de texto. 
...
El script es [`extract_subtitles.sh`](../extract_subtitles.sh) , se ejecutaría con `extract_subtitles.sh 2025`   
...
Para que funcione este script el archivo `2025_cachitos.mkv` tiene que estar en el directorio `video`, en años anteriores me bajaba el video directamente con el script, en este no. 
